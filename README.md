...
미니모는 드래그 앤 드롭으로 쉽고 빠르게 트랜디한 웹을 만들 수 있는 웹 빌더입니다.

처음엔 3D, 실시간 인터렉션 등을 포함한 재밌는 요소들을 블록단위로 제공하고 싶었습니다.

하지만 2주의 짧은 개발 기간 때문에, 이번 2주 동안엔 아래와 같은 목표를 갖고 진행하였습니다.

- 추후에 편하게 추가적인 블록(에디터의 기능) 및 템플릿을 추가할 수 있게 확장성 있는 구조로 만들기
...
- 초기 설계단계에서는 아래의 '기존 코드 데모'처럼 블록들을 이동 시 애니메이션이 없어도 괜찮을 것 같았습니다.
  하지만 막상 만들어보니 이는 보기에 어색했고, 사용자의 경험을 좋게하기 위해 애니메이션을 추가하기로 했습니다.
  다만 이미 개발 마무리 단계였기 때문에 기존에 완성된 에디터 및 블록 로직은 건드리지 않고 싶었고 애니메이션 관련 로직이기 때문에 컴포넌트/비즈니스 로직과 분리시키고 싶었습니다.

  따라서 아래의 `AnimateBlock 추가한 코드`처럼, `AnimateBlock`컴포넌트를 만들고, 그 `children`으로 블록들을 받아 애니메이션이 생기도록 구현하였습니다.
  `AnimateBlock`은 `children`의 `unique key`값을 기준으로 변화된 엘리먼트만 찾아 애니메이션을 하기 때문에, 여러 곳에서 재사용성 있게 사용할 수 있다고 생각합니다.

  해당 기능을 만들면서 처음으로 `useLayoutEffect`, `React.Children` 등을 이용하였는데, 그동안 리액트를 많이 공부했다고 생각했지만 여전히 모르는 것이 많다는 것을 느꼈고, 공식문서를 자주 봐야한다는 것 또한 느꼈습니다.
...
### 3. DRAG AND DROP
  ...
  라이브러리 없이 리액트에서 드래그 앤 드롭을 구현하는 글들을 찾아보면 대부분 컴포넌트 자체를 `draggable`하게 하고 드래그 앤 드롭을 합니다.
  하지만 이는 문제가 있었습니다. `draggable`컴포넌트 내부에 `text`가 있으면, 간헐적으로 그 `text`가 드래그 된다는 것이었습니다.

  유저가 잘못해서 `text`를 드래그 드롭하면 `text`는 `data-transfer`에 필요한 정보를 갖고 있지 않아 에러가 났습니다.
  물론 이 에러는 핸들링이 가능하며 `css`를 통해 `text`요소에 드래그가 안되게 할 수도 있었습니다.
  하지만 드래그 할 대상이 `text` 외에도 여러 요소드를 담고 있을 수도 있고, 추후에 블록들을 추가할 때마다 신경쓸 게 많아 보였습니다.

  따라서 이 문제는 블록마다 해당하는 이미지를 만들고, 그 이미지를 `draggable`하게 해서 해결하기로 했습니다.
  이미지는 단일 컴포넌트로 내부에 다른 요소들을 담고 있지 않기 때문에 문제를 원천적으로 차단할 수 있다고 생각했으며, 블록 추가도 용이하다고 생각했습니다.
  또한 드래그 할 블록들은 사실 보여주는 용도일 뿐, 실제로 사용하는 요소가 아니기에 목적에도 더 맞는 구현이라는 생각이 들었습니다.

  이를통해 꼭 코딩으로만 문제를 해결할 필요는 없다는 걸 배웠습니다.
  그 문제의 본질을 파악하려고 노력하고 그에 맞는 해결 방법을 제시하는 것이 결국 더 나은 서비스를 만드는게 아닐까 생각이 들었습니다.
